...
class EmojiUnicodeVersionTestCase(unittest.TestCase):
    ...
    def test_unicode_version_emoji_data_file(self) -> None:
        mq = itb_emoji.EmojiMatcher(
            languages = ['en_US'])
        self.assertEqual(mq.emoji_version('☺'), '0.6')
        self.assertEqual(mq.unicode_version('☺'), '1.1')
        self.assertEqual(mq.emoji_version('🤿'), '12.0')
        self.assertEqual(mq.unicode_version('🤿'), '12.0')
        self.assertEqual(mq.emoji_version('⚧'), '13.0')
        self.assertEqual(mq.unicode_version('⚧'), '4.1')

    def test_unicode_version_emoji_sequences_file(self) -> None:
        mq = itb_emoji.EmojiMatcher(
            languages = ['en_US'])
        self.assertEqual(mq.emoji_version('🇿🇼'), '2.0')
        self.assertEqual(mq.unicode_version('🇿🇼'), '8.0')
        self.assertEqual(mq.emoji_version('🤳🏽'), '3.0')
        self.assertEqual(mq.unicode_version('🤳🏽'), '9.0')
        self.assertEqual(mq.emoji_version('🤲🏿'), '5.0')
        self.assertEqual(mq.unicode_version('🤲🏿'), '10.0')
        self.assertEqual(
            mq.emoji_version(
    ...
    def test_unicode_version_emoji_zwj_sequences_file(self) -> None:
        ...
        self.assertEqual(mq.emoji_version('🏳\ufe0f\u200d\u26a7\ufe0f'), '13.0')
