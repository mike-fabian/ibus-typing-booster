...
def get_active_window() -> Tuple[str, str]:
    ...
    Currently works only in X11 sessions.
    '''
    program_name = ''
    window_title = ''
    ...
    try:
        ...
        LOGGER.exception(
            'Exception when calling xprop: %s: %s',
             error.__class__.__name__, error)
        return (program_name, window_title)
    # result now looks like in this example:
    #
    # b'_NET_ACTIVE_WINDOW(WINDOW) 0x1e02d79'
    id = result.stdout.split()[-1:][0]
    try:
        result = subprocess.run(
            [xprop_binary, '-id', id, '-f', 'WM_CLASS', '0s', 'WM_CLASS'],
            encoding='utf-8', stdout=subprocess.PIPE)
    except Exception as error:
        LOGGER.exception(
            'Exception when calling xprop: %s: %s',
    ...
    try:
        result = subprocess.run(
            [xprop_binary,
             '-id', id, '-f', '_NET_WM_NAME', '0t', '_NET_WM_NAME'],
            encoding='utf-8', stdout=subprocess.PIPE)
    except Exception as error:
        LOGGER.exception(
            'Exception when calling xprop: %s: %s',
